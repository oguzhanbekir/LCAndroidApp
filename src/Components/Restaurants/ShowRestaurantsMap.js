...
import { View, StyleSheet, PermissionsAndroid, TouchableOpacity, Dimensions  } from 'react-native';
...
class ShowRestaurantsMap extends React.Component {
  ...
  componentDidMount() {
    ...
    httpClient
    .get('/web/Restaurants/GetRestaurants')
    .then(res => {
              ...
              <Fragment>
                <MapView
                  ...
                  {this.state.restaurants.map(marker => (
                    <Marker
                      ...
                      description={_.find(marker.storeWorkingTimes, { 'dayOfWeek': CURRENT_DAY}).startTime+" - "+_.find(marker.storeWorkingTimes, { 'dayOfWeek': CURRENT_DAY}).endTime}
                    />
                  ))}     
               </MapView>  
               <View style={styles.buttonView}>
                  <TouchableOpacity 
                    ...
                    <Icon size={20} color={colors.showMyLocationIcon} name="my-location" type="MaterialIcons"/>
                  </TouchableOpacity>
                </View>
              </Fragment>  }  
            </View>
